# `pkgs.signStable drv`: re-sign a package's .app bundles and Mach-O executables
# with a long-lived self-signed cert so their code-signing designated
# requirement stops changing between rebuilds.
#
...
# Identity on Linux so generic modules need no platform branch.
{
  jl.overlays = [
    (final: prev: {
      signStable = let
        crt = ./_sign-stable/nix-local.crt;
        key = ./_sign-stable/nix-local.key;
      in
        drv:
          if !final.stdenv.hostPlatform.isDarwin
          then drv
          else
            final.runCommand "${drv.name}-signed" {
              nativeBuildInputs = [final.rcodesign final.file];
              meta = drv.meta or {};
