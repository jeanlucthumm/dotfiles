...
fp @ {jlib, ...}: {
  flake.modules.nixos.graphical = {pkgs, ...}: {
    # TODO this requires desktop host to have its own `pkgs` eval unless we want to make this the
    # norm for all x86 hosts
  ...
  flake.modules.homeManager.graphical = jlib.mkHomeManager {
    darwin = {pkgs, ...}: let
      ...
      vadDictation =
        pkgs.writers.writePython3Bin "vad-dictation" {
    ...
    nixos = {pkgs, ...}: let
      ...
      #   tiny, tiny.en, base, base.en, small, medium, large-v3, large-v3-turbo
      model = "base.en";
      silenceMs = 1000; # VAD: silence duration before transcription
