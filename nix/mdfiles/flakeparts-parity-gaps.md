...
Punch list of where the `flakeparts` branch (the `autoimport/` tree) diverged from
`master` (the old `nix/system` + `nix/home` layout). Produced by a parallel audit
(9 domain verifiers + a completeness critic) comparing every master module against
its autoimport counterpart, plus hands-on eval debugging of each broken host.

**The bulk of the migration is clean.** Verified parity-preserving: nushell
(all `.nu` scripts byte-identical), taskwarrior config, hyprland/niri/hyprlock HM
config, ssh (nixos+darwin+HM), backups/sanoid, monitoring/netdata, torrenting,
nginx route distribution, darwin hotkeys/hammerspoon, theme/stylix, agenix
home-secrets, containers, amd-gpu, tailscale. The gaps below are the exceptions.

Read master files with `yadm show master:nix/<path>`.

---

## Decisions already made (do NOT "restore" these)

- **fish → retired.** You drive nushell. Don't re-add `programs.fish`. Just clean
  up the now-dangling `enableFishIntegration` flags (see Low §L1). Note: the fish
  git/task abbrs (`g`/`ga`/`gm`/`ta`/`tr`/…) were never ported to nushell — port
  separately only if you miss them.
- **neo4j → dropped cleanly.** It was already half-broken on master (the
  `jeanluc-neo4j.age` file never existed there). Don't restore the service; just
  kill the dangling `get-key-neo4j` ref (see §H-neo4j).
- **moltbot/openclaw → deferred, stays off.** Upstream is broken; you don't want it
  running on the server. Leave it not-hooked-up for now; cleanup tracked separately
  (see §Deferred).
- **darkMode → keep light.** The `darkMode = false` on desktop/macbook/macbook-work
  is intentional. Don't flip back to dark.

---

## Eval-breakers (these hosts do not build today)

### E1. `cloud-vm` — multiple eval errors (homeConfiguration)
File: `autoimport/modules/hosts/cloud-vm.nix`. This is a standalone
`homeManagerConfiguration`, which is NOT checked by `nix flake check`, so the breakage
is latent. Three stacked problems:

1. **`lib` unbound** — `programs.git.signing.signByDefault = lib.mkForce false` lives
   in a bare attrset module `{ ... }` whose enclosing fn only binds `fp:`. Make the
   inline module a function: `({lib, ...}: { ... })`.
2. **`pkgs` not passed** — `homeManagerConfiguration` requires `pkgs`. Master passed
   `nixpkgs.legacyPackages.x86_64-linux` + overlays. Use the per-system configured
   pkgs:
   ```nix
   fp @ {withSystem, ...}: {
     flake.homeConfigurations."developer@cloud-vm" = withSystem "x86_64-linux" ({pkgs, ...}:
       fp.inputs.home-manager.lib.homeManagerConfiguration {
         inherit pkgs;
         modules = with fp.config.flake.modules.homeManager; [ base dev (...) ];
       });
   }
   ```
3. **`osConfig` required by darwin blocks** — once it evaluates, the `base`/`dev`
   slots pull nushell modules whose `darwin` mkHomeManager blocks take `osConfig` as
   a *required* arg. Standalone HM has no `osConfig`, and `mkHomeManager`'s gate calls
   the fn even on Linux. Fix by defaulting `osConfig ? null` in BOTH:
   - `autoimport/modules/nushell/darwin-path-fix.nix:10`
   - `autoimport/modules/nushell/nushell.nix:89`  (`darwin = {osConfig ? null, ...}:`)
   (Bodies are darwin-gated, so null is never dereferenced on Linux.)
4. **taskwarrior gcp secret** — see §M-task; cloud-vm pulls taskwarrior via `dev`
   but has no agenix secret. Same guard fixes it.
