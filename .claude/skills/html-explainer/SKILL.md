...
- Save the file in `~/memory/projects/explainers/` (durable), then show it in a new kitty tab: `kitten @ launch --type=tab --tab-title "<title>" terminal-browser open <path>`, where `<title>` is a short human-readable subject title (2-3 words, spaces fine, e.g. `Deploy Pipeline`). If invoked with `--split`, open a vertical split next to the current pane instead: `terminal-browser open <path> --split right --size 0.4`. See the terminal-browser skill for driving the pane and gotchas. Also output the direct file URL (`file:///Users/jeanlucthumm/memory/projects/explainers/<file>.html`) as a Chrome fallback, and copy the absolute path to the clipboard (`printf %s "$path" | pbcopy`) and tell the user it's there.
